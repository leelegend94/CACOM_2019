...
def plot_and_save(data,name):
	plt.plot(data)
	plt.savefig('/Users/zoe/Desktop/CACOM_data_plot/floatline_ft/'+'{}'.format(name)+'.jpg') # change to your own path
	plt.close()
...
def load_all_data(path):
	...
	for i_case, case_name in enumerate(case_list_copy):
		...
		if len(new_data_floatline) == 1000:
			...
			#plot_and_save(new_data_floatline, case_name)
			...
			#plot_and_save(ft_floatline[i_case,:], case_name)
...
all_label_floatline = np.concatenate((np.zeros((len(ab_floatline_ft),1)), np.ones((len(nor_floatline_ft),1))), axis=0).ravel()
...
model, fold_data = classify(all_data_floatline, all_label_floatline)
